  ...
  try {
    ...
    if (
      ...
      res.status(400).json({
        success: false,
        message:
          "Customer name, phone, email, location, booking date and plan are required",
      });
      return;
    }
 ...
 */
export const attachPaymentScreenshot = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (
      ...
      res.status(400).json({
        success: false,
        message:
          "Payment screenshot URL is required",
      });
      return;
    }
    ...
    console.error(
      "Attach payment screenshot error:",
      error
    );
    ...
    res.status(500).json({
      success: false,
      message:
        "Failed to attach payment screenshot",
    });
  }
};